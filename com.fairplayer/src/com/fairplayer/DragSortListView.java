...
public class DragSortListView extends ListView {
    ...
    private int getShuffleEdge(int position, int top) {
        ...
        if (mSecondExpPos <= mSrcPos) {
            ...
            if (position == mSecondExpPos && mFirstExpPos != mSecondExpPos) {
                ...
                if (position > mSecondExpPos && position <= mSrcPos) {
    ...
    private boolean updatePositions() {
        ...
        if (mAnimate) {
            ...
            if (mFloatViewMid < slideEdgeTop) {
                ...
                if (mFloatViewMid < slideEdgeBottom) {
        ...
        if (mFirstExpPos < numHeaders) {
            ...
            if (mSecondExpPos >= getCount() - numFooters) {
     ...
     * through to DropListener#drop(int, int).
     *
    ...
    private void continueDrag(int x, int y) {
        ...
        if (minY > mLastY && minY > mDownScrollStartY && currentScrollDir != DragScroller.DOWN) {
            ...
            if (maxY < mLastY && maxY < mUpScrollStartY && currentScrollDir != DragScroller.UP) {
                ...
                if (maxY >= mUpScrollStartY && minY <= mDownScrollStartY
    ...
    private int adjustScroll(int movePos, View moveItem, int oldFirstExpPos, int oldSecondExpPos) {
        ...
        if (movePos <= oldFirstExpPos) {
            ...
            if (movePos == oldSecondExpPos) {
                if (movePos <= mFirstExpPos) {
                    adjust += moveBlankBefore - maxBlank;
                } else {
                    if (movePos == mSecondExpPos) {
     ...
     * to #startDrag(int,View,int,int,int) after obtaining
     ...
     * ~ #DRAG_NEG_X</code> to allow dragging the floating
    ...
    private void updateFloatView() {
        ...
        if ((mDragFlags & DRAG_POS_X) == 0 && floatX > padLeft) {
            ...
            if ((mDragFlags & DRAG_NEG_X) == 0 && floatX < padLeft) {
                mFloatLoc.x = padLeft;
            }
        }
        ...
        if (floatY < topLimit) {
            ...
            if (floatY + mFloatViewHeight > bottomLimit) {
     ...
     * your own is not passed, the default DragSortFloatViewManager
    ...
    public interface FloatViewManager {
         ...
         * and the offset provided in DragSortListView#startDrag.
     ...
     * ListAdapter#hasStableIds(), you will need to call
     * #moveCheckState(int, int) to move the check boxes along with the
     * list items.
     *
     * @param l
     */
     ...
     * ListAdapter#hasStableIds()). This is because without IDs, the
     ...
     * DragSortListView#setDragScrollProfile(DragScrollProfile) to
     * set custom profile.
     *
     * @author heycosmo
     *
