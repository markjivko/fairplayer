 ...
 * DragSortFloatViewManager, which provides basic float View
 * creation.
 *
 ...
 */
public class DragSortController extends DragSortFloatViewManager implements View.OnTouchListener, GestureDetector.OnGestureListener {
     ...
     * #dragHandleHitPosition(MotionEvent). Override
        ...
        if (mCanDrag && !mDragging && (mHitPos != MISS || mFlingHitPos != MISS)) {
            if (mHitPos != MISS) {
                if (mDragInitMode == ON_DRAG && Math.abs(y2 - y1) > mTouchSlop && mSortEnabled) {
                    ...
                    if (mDragInitMode != ON_DOWN && Math.abs(x2 - x1) > mTouchSlop && mRemoveEnabled) {
                        mIsRemoving = true;
